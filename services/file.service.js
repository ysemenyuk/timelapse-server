...
const makeDir = (pathToDir) => {
  ...
  return fsp.mkdir(fullPath).catch((e) => console.log(`catch makeDir pathToDir error: ${e.message}`));
};
...
const writeFile = (pathToFile, data = '') => {
  ...
  return fsp.writeFile(fullPath, data).catch((e) => console.log(`catch writeFile pathToFile error: ${e.message}`));
};

const readFile = (pathToFile) => {
  console.log('readFile pathToFile - ', pathToFile);
  const fullPath = path.join(pathToFiles, pathToFile);
  return fsp.readFile(fullPath).catch((e) => console.log(`catch readFile pathToFile error: ${e.message}`));
};

const removeDir = (pathToDir) => {
  console.log('removeDir pathToDir - ', pathToDir);
  const fullPath = path.join(pathToFiles, pathToDir);
  return fsp.rmdir(fullPath, { recursive: true }).catch((e) => console.log(`catch removeDir error: ${e.message}`));
};
